    ...
    .map((g) => {
      const rows = g.alerts
        .map(
          (a) => `
      <tr>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;">${a.title}</td>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;">
          <span style="background:#f3f4f6;border-radius:4px;padding:2px 6px;font-size:12px;font-weight:600;">${a.status}</span>
        </td>
        <td style="padding:6px 8px;border-bottom:1px solid #e5e7eb;color:#6b7280;font-size:12px;">${g.jurisdiction}</td>
      ...
      return `
    <h3 style="font-size:14px;color:#374151;margin:20px 0 6px;">${g.jurisdiction}</h3>
...
<body style="font-family:sans-serif;color:#1a1a1a;margin:0;padding:24px;">
  <div style="max-width:640px;margin:0 auto;">
    ...
    <p style="font-size:16px;font-weight:bold;margin:0 0 4px;">${orgName}</p>
    ...
    ${groupHtml}
    <p style="color:#9ca3af;font-size:12px;margin-top:32px;">You are receiving this because you are an owner of ${orgName} on RegWatch.</p>
  </div>
</body>
</html>`;
